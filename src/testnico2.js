...
async function main() {
  ...
  // UPDATE/CREATE GRAPH FUNCTION
  await updatemap(SelectedDates)
  async function updatemap(SelectedDates) {
    const datefilteredplanning = d3.filter(
      planningParsed, d => d3.utcFormat('%A %e %B %Y')(d.date) == d3.utcFormat('%A %e %B %Y')(SelectedDates[0]))
